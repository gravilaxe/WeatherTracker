...
final class WeatherAPIClient: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    func fetchWeather() async {
        ...
        guard let url = URL(string: "https://api.tomorrow.io/v4/timelines?location=\(location.coordinate.latitude),\(location.coordinate.longitude)&fields=temperature&fields=weatherCode&units=metric&timesteps=1h&startTime=\(dateFormatter.string(from: Date()))&endTime=\(dateFormatter.string(from: Date().addingTimeInterval(60 * 60)))&apikey=2nJFaA946FTcFf0HV4gfrWUklkmWUhy1") else {
            return
        }
